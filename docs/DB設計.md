...
Cloudflare Workers KV (Key-Value Store) を、**一時的な状態管理（Temporary State Management）** のために補助的に利用しています。
DB (D1) とは明確に役割を分担しています。

| ストア種別         | 名称                       | 用途                                                                           | データの寿命 (TTL)      |
| :----------------- | :------------------------- | :----------------------------------------------------------------------------- | :---------------------- |
| **D1 (Database)**  | `line-audio-summarizer-db` | ユーザー情報、設定、Inboxなど「永続的かつ構造化されたデータ」の管理。          | 永続 (ユーザー退会まで) |
| **KV (Key-Value)** | `LINE_AUDIO_KV`            | セットアップフローの進行状況、レート制限のカウンタなど「一時的な状態」の管理。 | 一時的 (数分〜数日)     |

### KVの使用例

- **セットアップ状態の保持**:
    - キー: `setup_state:{userId}`
    - 値: `waiting_for_obsidian`, `waiting_for_webhook`
    - TTL: 1時間〜1日
    - 説明: ユーザーが「Obsidian連携」ボタンを押した後、QRコードを読み込むまでの間の状態を保持します。TTLを設定することで、途中で離脱しても自動的に状態がリセットされ、ゴミデータが残りません。

- **モード設定フロー**:
    - キー: `prompt_setting_state:{userId}`
    - 値: `waiting`
    - TTL: 5分
    - 説明: 「カスタムプロンプト入力」待ちの状態を管理します。
