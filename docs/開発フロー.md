...
- [ ] **クライアントサイド鍵管理 & 設定**
    - [ ] `crypto/index.ts`: Web Crypto API を用いた RSA 鍵ペア生成の実装。
    - [ ] `settings.ts`: Settings Tab の実装 (User ID 入力、鍵生成、鍵登録)。
    - [ ] 秘密鍵のローカル保存 (IndexedDB or LocalStorage)。
- [ ] **API クライアント & 同期**
    - [ ] `api/index.ts`: Backend API との通信ロジック。
    - [ ] `sync/index.ts`: `GET /api/inbox` から取得し、ローカルで復号。
- [ ] **ファイル保存ロジック**
    - [ ] 復号したテキストを Markdown ファイルとして保存。
    - [ ] リボンアイコン等のトリガー実装。
    - [ ] テンプレート置換処理。
