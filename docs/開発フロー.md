> [!IMPORTANT]
> **AIアシスタントへの指示**:
> 開発を進める際は、必ず本ドキュメント（\`docs/開発フロー.md\`）と \`docs/要件定義.md\` を読み込み、現在の進捗ステップを確認してください。
...
- [ ] **ディレクトリ構造の変更**
    - [ ] ルート直下のプラグイン関連ファイル (`src/`, `main.ts`, `manifest.json`, `styles.css`, `esbuild.config.mjs` 等) を `packages/obsidian-plugin/` へ移動。
    - [ ] `packages/backend/` ディレクトリを作成。
    - [ ] `shared/` ディレクトリを作成。
    - [ ] ルートに `package.json` (ワークスペース定義用) を作成。
    - 参照: `docs/要件定義.md` - ## 2. ディレクトリ構成
- [ ] **Cloudflare Workers の初期化**
    - [ ] `packages/backend/` 内で Wrangler を設定 (`wrangler.init` 相当)。
    - [ ] `pnpm` ワークスペースの設定 (推奨) または `npm` ワークスペースの設定。
...
- [ ] **Backend 環境設定**
    - [ ] `packages/backend/wrangler.toml` の設定 (KV, D1 のバインディング定義)。
    - [ ] `packages/backend/src/index.ts` (エントリポイント) の作成。
- [ ] **LINE Webhook 受け口の実装**
    - [ ] `handlers/webhook.ts` の実装。
    - [ ] 署名検証ロジック (`x-line-signature`) の実装。セキュリティ必須項目。
    - 参照: `docs/要件定義.md` - ## 4. セキュリティ要件
- [ ] **デプロイとWebhook URLの確認**
    - [ ] `wrangler deploy` (または開発用トンネル) で公開 URL を発行。
...
- [ ] **疎通確認 (Echo Bot)**
    - [ ] LINE からメッセージを送り、ログ (`wrangler tail`) または簡単な応答が返るか確認。
